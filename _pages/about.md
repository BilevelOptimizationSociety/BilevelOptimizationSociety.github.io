---
layout: about
title: The BOS
permalink: /
subtitle: "<i> The Bilevel Optimization Society since 2025.</i>"
sidebar_nav: true
nav_order: 0

profile:
  align: right
  image: prof_pic.png
  image_circular: false

announcements:
  enabled: true
  scrollable: false
  limit:

latest_posts:
  enabled: true
  scrollable: false
  limit:
---

The bilevel optimization society (BOS) is a technical section of the Mathematical Optimization Society (MOS). The goal of the BOS is to create and maintain a community of researchers that recognize their work as related to bilevel optimization, while at the same time expanding its basis and promoting its outreach within and outside academia. This is achieved through the organization of:

- the [International Conference on Bilevel Optimization (ICBO)](/icbo/2026/index.html) series;
- the organization of the bi-weekly [BOS Webinar](https://bileveloptimization.org/bos_webinar/index.html); and,
- the dissemination and promotion of research and information related to bilevel optimization.

#### Upcoming Events

No new events to show up to now.

- ~~Jul 08 - Jul 10, 2026:** [EUROPT 2026 – Bilevel Optimization Stream](https://www.jku.at/europt2026/)~~
- ~~Aug 02 - Aug 05, 2026:** [International Conference on Bilevel Optimization – ICBO 2026](/icbo/2026/index.html)~~
- ~~Oct 06 - Oct 10, 2025: [Kaiserslautern Applied and Industrial Mathematics Days – KLAIM 2025](https://www.itwm.fraunhofer.de/en/fairs_events/2025/2025_10_06_klaim_en.html)~~

If you want to promote your bilevel session in a conference, please [email us](mailto:bileveloptimizationsociety@gmail.com).
